...
从 GitHub Release 下载 `luci-app-performance-manager-all-1.0.0_rc14-r1.apk` 后，只需安装这一份应用 APK：

仓库的官方 OpenWrt SDK 编译仍会编译并校验所有拆分包以及实体一体化包；仅 GitHub Release 为了下载方便只提供一体化 APK 作为公开安装资产。

```sh
apk add --allow-untrusted /tmp/luci-app-performance-manager-all-1.0.0_rc14-r1.apk
```
...
> `1.0.0-rc.14` 只代表待验证源码候选，并将经逐文件验证的一体化 APK 作为唯一公开安装资产。只有同提交的官方 SDK/APK、精确 Rill、Generic/Hyper-V/KVM、真实 A/B、sysupgrade、生命周期及 24 小时 Rill-present soak 全部由 Stable 聚合器判为 PASS，才允许发布 `v1.0.0`。
