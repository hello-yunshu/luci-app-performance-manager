...
check("Core exact linked versions", "RILL_LINKED_RILL_ML_VERSION = '1.5.1'" in CORE and "RILL_PINNED_ADAPTER_VERSION = '1.0.1'" in CORE)
...
check("PM adapter exact registry dependency", 'version = "=1.5.1"' in adapter_dependency and "git" not in adapter_dependency and "path" not in adapter_dependency)
check("PM adapter binary/package identity", DEP["adapter"]["owner"] == "hello-yunshu/luci-app-performance-manager" and DEP["adapter"]["binary"] == "performance-manager-rill-adapter" and DEP["adapter"]["version"] == "1.0.1")
check("RillML exact dependency identity", DEP["rillMl"] == {"package": "rill-ml", "registry": "crates.io", "version": "1.5.1", "resolution": "exact", "features": ["serde"]})
