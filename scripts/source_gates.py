...
phases['8']=gate('Rill Intelligence (external runtime)',[
 ...
 ('protocol major gate wiring',all_tokens(CORE,['RILL_PROTOCOL_API','protocol-major-mismatch','(r.response?.api ?? 0) != RILL_PROTOCOL_API'])),
 ...
 ('per-op send/status/observe/outcome wiring',all_tokens(CORE,['rill_send','rill_status','rill_observe','rill_outcome_payload']))])
phases['8'].update({'evidence':'structural: PM-side fail-closed contract wiring present; Rill unavailable/malformed fail-closed and status honesty are verified by the runtime harness [7] and scripts/rill_contract_check.py. Upstream external integration is BLOCKED (see docs/rill-integration-status.json).'})
phases['9']=gate('Recommend',[
 ('learned advisory wiring',all_tokens(CORE,['learnedAdvisory','rill.detail?.recommendations']))])
...
phases['10']=gate('Assisted Auto',[
 ('default off',"option assisted_auto '0'" in CFG),('double opt-in',all_tokens(CORE,["!= 'assisted'","bool_cfg('main.assisted_auto', false)"])),('maintenance + health gates wiring',all_tokens(CORE,['in_maintenance_window()','system_guard()'])),('action selected before traffic gate',CORE.index('let action = actions[0]')<CORE.index('assisted_low_traffic(current, target_ref?.runtimeName ?? null)')),('target-specific low-traffic gate wiring',all_tokens(CORE,['assisted_low_traffic(current, runtime)','assisted-previous-','resolve_target(action.applyTarget)'])),('safe allowlist only','index(SAFE_ACTIONS, action.id)' in CORE)])
