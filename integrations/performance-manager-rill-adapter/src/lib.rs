//! `performance-manager-rill-adapter` — the `pm-rill-shadow` protocol v1 decision host.
//!
...
//! Performance Manager repository. It implements the independent `pm-rill-shadow` wire
...
impl AdapterState {
    ...
    /// Load persisted state from disk, or start fresh only when absent. A
    /// corrupt, oversized, or incompatible state file fails closed so a
    /// deployment can repair or restore it without silently losing learning.
    pub fn from_disk(config: AdapterConfig, now_ms: u64) -> Result<Self, AdapterError> {
        ...
        if metadata.len() > MAX_STATE_FILE_BYTES {
            return Err(AdapterError::new(
                ErrorCode::StateLoad,
                "state file exceeds the maximum accepted size",
            ));
        }
        ...
        let persisted: PersistedState = serde_json::from_slice(&bytes).map_err(|error| {
            AdapterError::new(
                ErrorCode::StateLoad,
                format!("invalid persisted state: {error}"),
            )
        })?;
        if persisted.schema_version != STATE_SCHEMA_VERSION {
            return Err(AdapterError::new(
                ErrorCode::StateLoad,
                format!("unsupported state schema: {}", persisted.schema_version),
            ));
        }
        Ok(Self {
            config,
...
mod tests {
    ...
    #[test]
    fn legacy_adapter_state_versions_migrate_without_reset() {
        for legacy_version in ["0.15.0", "0.15.0"] {
            let dir = std::env::temp_dir().join(format!(
                "pm-shadow-legacy-state-{}-{}",
                std::process::id(),
                legacy_version.replace('.', "_")
            ));
            let cfg = AdapterConfig {
                state_dir: dir.clone(),
                ..config()
            ...
            let body = handle_request(
                &mut state,
                &serde_json::to_vec(&observe_request("obs-legacy")).unwrap(),
                10_000,
            );
            let response: serde_json::Value = serde_json::from_slice(&body).unwrap();
            let decision_id = response["decisionId"].as_str().unwrap().to_owned();
            handle_request(
                &mut state,
                &serde_json::to_vec(&outcome_json(&decision_id)).unwrap(),
                20_000,
            );
            ...
            let legacy = raw.replace(
                ...
                &format!("\"adapterVersion\":\"{legacy_version}\""),
            );
            std::fs::write(&path, legacy).unwrap();
            let reloaded = AdapterState::from_disk(cfg.clone(), 30_000).unwrap();
            let (_, health) = reloaded.status(31_000);
            assert_eq!(health.total_samples, 1);
            assert!(reloaded.state_file_path().exists());
            let _ = std::fs::remove_dir_all(&dir);
        }
    }
