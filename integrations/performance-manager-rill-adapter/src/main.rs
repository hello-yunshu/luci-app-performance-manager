//! `performance-manager-rill-adapter` daemon entrypoint.
//!
...
mod unix {
    ...
    use performance_manager_rill_adapter::{
    ...
    #[derive(Debug, Parser)]
    #[command(
        name = "performance-manager-rill-adapter",
        version,
    ...
    pub fn entry() -> ! {
        ...
        if let Err(error) = run(args) {
            eprintln!("performance-manager-rill-adapter: fatal: {error}");
            std::process::exit(1);
        }
        std::process::exit(0);
    }
    ...
    fn run(args: Args) -> Result<(), Box<dyn std::error::Error>> {
        ...
        loop {
            ...
            match listener.accept() {
                Ok((stream, _)) => {
                    ...
                    std::thread::spawn(move || {
                        if let Err(error) = serve_connection(stream, state) {
                            eprintln!(
                                "performance-manager-rill-adapter: connection error: {error}"
                            );
                        }
                    });
                }
                ...
                Err(error) => {
                    eprintln!("performance-manager-rill-adapter: accept error: {error}");
                    std::thread::sleep(std::time::Duration::from_millis(50));
                }
            }
        }
        ...
        loop {
            match read_frame(&mut stream, max_message) {
                ...
                Ok(Frame::Line(raw)) => {
                    ...
                    let response = performance_manager_rill_adapter::handle_request(
                        &mut state,
                        &raw,
                        now_ms(),
                    );
...
fn main() {
    ...
    {
        eprintln!(
            "performance-manager-rill-adapter: this build supports Unix-like systems only \
