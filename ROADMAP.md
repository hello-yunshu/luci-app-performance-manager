...
Planning pack v0.3.2 is contract-frozen. `1.0.2` retains the exact Rill/evidence closure and makes the byte-verified all-in-one APK plus target-specific adapter APK the public install assets, with every repository-owned runtime layer and compiled Simplified Chinese translation covered by the SDK build. Rill remains an external runtime dependency for the Core; the PM-owned adapter is built separately from exact crates.io `rill-ml`. The remaining work is execution of the declared same-commit validation matrix, not feature expansion.
...
6. Freeze `1.0.2` Stable only when all evidence is attached and independently auditable.
