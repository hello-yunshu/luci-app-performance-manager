...
- **CI restructured:** `.github/workflows/ci.yml` now has `static` (incl. LuCI render smoke), `rill-contract` (contract + pinned upstream provenance, no Rust), `openwrt-ucode` (official 25.12.5 rootfs compile of Core) and `openwrt-sdk-build` (official SDK builds only the packages this repository owns). Removed `rill-native` and `openwrt-sdk-build-rill`.
